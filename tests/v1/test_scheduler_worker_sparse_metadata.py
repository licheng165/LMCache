...
class TestBuildConnectorMetaSparseSyntheticLoadSpec:
    ...
    def test_mtp_scratch_probe_guard_skips_unsafe_window_load(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        impl = _make_scheduler_impl()
        impl._decode_window_save_window_size = 256
        impl._vllm_config = SimpleNamespace(
            model_config=SimpleNamespace(hf_config=SimpleNamespace(index_topk=256)),
            num_speculative_tokens=1,
        )
        monkeypatch.setenv("LMCACHE_MTP_DW_UNSAFE_SCRATCH_GUARD", "1")

        req_id = "mtp-guard"
        prompt = list(range(356))
        decode_tokens = list(range(10_000, 10_158))
        all_tokens = prompt + decode_tokens
        vllm_req = SimpleNamespace(
            request_id=req_id,
